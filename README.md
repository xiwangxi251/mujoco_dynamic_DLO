...
Panda 内指垫保持 Menagerie 原始尺寸、位置和碰撞形状，只把材料改为高摩擦橡胶：
`condim=6`，滑动/扭转/滚动摩擦分别为 `6.0 / 0.35 / 0.12`。没有扩大碰撞盒或增加
凹槽，因此策略仍需把 28 mm 线缆准确对准官方约 17 mm 的主指垫。闭爪执行器增益为
Menagerie 配置的 5 倍；这是始终生效的执行器/接触参数，不随抓取状态、策略阶段或成功
判定切换。
