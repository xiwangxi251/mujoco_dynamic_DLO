...
### Attempt 3b — arc_aux (dense s as auxiliary only): NEW BEST (modest)

`arc_aux`: same per-point s head but only as a **dense auxiliary task** —
direct node regression remains the output path. S_sc sequential eval
(strict ordered MPNE, mm):

| variant | static | rigid | shape | combined | frame-weighted |
|---|---|---|---|---|---|
| `dual_local_data` | 4.0 | 3.3 | 29.0 | 21.3 | 14.6 |
| `arc_aux` | **3.7** | **3.2** | **28.0** | **20.5** | **14.1** |
| `dual_local_xsamp` | 5.2 | 4.3 | 31.5 | 20.4 | 14.9 |

Dense per-point supervision improves all four scenarios (consistent but
small; val-diagnostic was actually slightly worse, 51.1 vs 49.5 — S_sc is
the primary metric). Latency 4.68 ms vs 4.54 ms, +0.03M params.

`dual_local_xsamp` (crossing-frame ×5 oversampling): NEGATIVE — shape
worsened 29.0→31.5 and static/rigid degraded; shifting the sampling
distribution toward hard frames hurt ordinary frames without fixing
strand assignment.

### Attempt 4 — top-down 2.5-D image representation: IN PROGRESS

Motivation (user-proposed): the residual error lives in XY topology
(shape high-error frames: X 80, Y 115, Z 10 mm), and humans trace the
cable easily on the planar projection. Replace the point-cloud encoder
with a 2.5-D raster + small UNet, keep the arc-coordinate idea as the
extraction path:

- Input raster (5, H=256, W=192), ~7 mm/px over the workspace window
  [-0.20,1.15]×[-0.95,0.85]: occupancy log-count, z_mean, z_min, z_max
  (two stacked strands remain separable), gaussian hand marker.
- GT dense maps (3,H,W): cable mask + arc coord s + z drawn along the
  full GT polyline incl. occluded parts → forces inpainting.
- `UNetSkeleton` (w=16, 1.96M params) → (mask logit, s, z) maps.
- Chain extraction: mask>0.5 pixels sorted by predicted s, resampled to
  14 nodes (xy = pixel-centre world coords, z = predicted z channel).

Pipeline note: per-sample rasterisation inside `__getitem__` made the
DataLoader IPC-bound (~1.6 MB/item → 0.5 s/batch, workers hurt).
Fixed by returning raw fields (~10 KB/item) and rasterising on GPU via
`rasterize_torch` (scatter_add/scatter_reduce) in the train loop —
~168 ms/batch incl. w16+AMP+channels_last step on TITAN RTX,
~12-14 min/epoch.

#### Attempt 4 results — extraction was the bottleneck, not the model

No-history UNet (`img_unet`, w16, 20 ep, val 52.4mm / IoU 0.715).
Sequential eval 5 held-out eps/scenario, strict ordered MPNE:

| extractor | static | rigid | shape | combined |
|---|---|---|---|---|
| s-binning | 20.4 | 21.4 | 70.0 | 44.1 |
| skeleton+bridge+temporal | 21.8 | 17.9 | 51.2 | 38.3 |

s-binning fails because disconnected mask regions sharing similar
predicted s average into off-mask centroids. Skeleton traversal
(skeletonize -> bridge_components gap<=18px -> graph diameter path ->
arc-length resample) eliminates off-mask nodes; direction anchored by
full-chain match to previous frame (endpoint-only matching fails on
C-shapes). Predicted-mask inpainting beats visible-occupancy masking
under identical extraction: 30.8 vs 58.8 mm on shape 20283215; mask
IoU 0.718 vs 0.242.

#### Attempt 4b — temporal self-feedback (`img_unet_hist`): mixed result

8-ch input = current 5 + previous frame's predicted (mask,s,z) maps.
Training prev = corrupted GT (shift +/-3px, s-flip p=0.15, dropout
p=0.05); eval feeds back model's own output (s,z masked by binarized
pred mask to match the zero-outside-line training distribution).
Val (teacher-forced): 47.7mm / IoU 0.798.

Orientation watchdog: temporal anchor can enter an absorbing state —
once flipped, wrong prev self-sustains (rigid 20282005: 185/185 flips,
365.8mm). s-map errors are intermittent, temporal errors sustained, so:
if temporal wants to flip against the s-oriented chain for >=25
consecutive frames, defect to s-orientation. Fixes lock without
regressing episodes where temporal is right (shape 20283215 max legit
conflict run = 20 < 25).

Sequential eval with streak-watchdog (5 eps/scenario):

| model | static | rigid | shape | combined |
|---|---|---|---|---|
| no-hist + watchdog | 21.8 | 17.9 | 48.1 | 37.5 |
| hist + watchdog | 21.6 | 30.7 | **35.0** | 37.6 |

Flip totals: shape 220->79, rigid 0->25, combined 34->41.

Occlusion-split mask recall (occluded GT pixels covered by pred mask):

| model | static | rigid | shape | combined |
|---|---|---|---|---|
| no-hist | 0.846 | 0.894 | 0.789 | 0.816 |
| hist | 0.600 | 0.771 | 0.757 | 0.781 |

FINDING: predicted-map feedback helps ORDERING (temporally consistent
s-map, fewer flips, best shape result) but HURTS occluded-geometry
recall — classic exposure bias: the model copies its own prior instead
of re-deriving from observation, worst exactly where the cable barely
moves (static/rigid). To use history for POSITION need raw-observation
history (no self-reinforcement) or true self-feedback training.
