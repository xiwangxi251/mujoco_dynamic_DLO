...
class EnvConfig:
    ...
    # 实验场景把“整体运动”和“局部形变”拆开。默认值严格保留原环境的
    # 去均值准周期形变力场，因此旧训练、旧命令和旧 checkpoint 不会暗中换任务。
    ...
    # 线缆 OOD 参数相对 XML 标称值缩放。非标称配置通过 MjSpec 在编译前真正
    # 改变长度、质量、弹性、阻尼和摩擦，而不只是写入日志标签。
...
class CableGraspEnv:
    ...
    def __init__(self, config: EnvConfig | None = None):
        ...
        # Menagerie 的夹爪位置伺服较软，持续线缆扰动会在抬升时逐渐撑开或滑出指垫。
        # 同比例提高位置增益和位置偏置只改变闭爪刚度，不改变目标开口和 forcerange；
        # 速度偏置按平方根缩放，以避免阻尼随刚度一起过度增加。
        ...
        for geom_id in self.pad_geom_ids:
            ...
            # 六维接触保留滑动、绕法线扭转和滚动摩擦。较高但有限的橡胶摩擦
            # 用来承受持续扰动；不改变指垫外形，也不向线缆施加跟随力。
        ...
        # 显式 episode seed 供跨方法配对评估使用。旧的连续 RNG 调用方式仍然保留，
        # 但正式 benchmark 应为每个回合传入独立 seed，避免依赖此前跑过多少回合。
    ...
    def _load_model(config: EnvConfig) -> mujoco.MjModel:
        ...
        # MjSpec 会先展开 composite，再编译模型，因此长度和材质变化会进入
        # 弹性插件、质量矩阵和接触参数；这不是只改 episode 元数据的“假 OOD”。
        ...
        for geom in spec.geoms:
            if not geom.name.startswith("cableG"):
                continue
            # composite 胶囊使用 fromto 表示每节长度，半径维持 14 mm 不变。
    ...
    def _shape_acceleration(self, t: float, p: float) -> np.ndarray:
        ...
        if self.config.motion_regularity == "regular":
            ...
            # 这是修改前的原始扰动公式。嵌套相位调制与互不整除频率使短 episode
            # 内不重复；保留它作为旧 checkpoint 的兼容默认条件。
            lateral = (
                2.30 * np.sin(
        ...
        # legacy_v1 逐式保留旧 checkpoint 训练时的场：按每轴算术去均值，
        # 不再投影力矩。正式 factorized_v1 才使用质量内积去掉完整刚体子空间。
    ...
    def _apply_cable_disturbance(self) -> None:
        ...
        # static 的定义是没有任何额外线缆外力；连软边界也不参与。
        confined = (
            intended
