...
for f in episode_frames(packed, scen, seed):
    ...
    if "prev" in histmode:
        # s-field temporal anchor: sample current s at the DEPLOYED chain's
        # node positions (prev has fixed material order 0->13). If s now
        # DECREASES along that order, the field flipped globally -> unflip
        # before extraction and before feeding back, so a flipped map
        # cannot self-sustain through prev_s.
        s_raw = om[1].copy()
        if prev is not None and np.isfinite(prev).all():
            rr = np.clip(((prev[:, 1] - Y0) / (Y1 - Y0) * H).astype(int),
                         0, H - 1)
            cc = np.clip(((prev[:, 0] - X0) / (X1 - X0) * W).astype(int),
                         0, W - 1)
            pmd = ndi.binary_dilation(pm, iterations=2)
            valid = pmd[rr, cc]
            if valid.sum() >= 8:
                sv = om[1][rr[valid], cc[valid]]
                iu = np.triu(np.ones((int(valid.sum()),) * 2, bool), 1)
                d = (sv[:, None] - sv[None, :])[iu]
                neg = (d > 0.05).mean()      # i<j but s_i>s_j: inverted
                pos = (d < -0.05).mean()     # i<j and s_i<s_j: consistent
                if neg > 0.6 and neg > 2 * pos:
                    # flip only for extraction; feed RAW s back as prev so
                    # a false positive cannot poison the feedback channel
                    om[1] = np.where(pm, 1.0 - om[1], om[1])
        bm = (torch.sigmoid(torch.from_numpy(om[0])) > 0.5).float()
        prev_map = torch.stack(
            [bm, torch.from_numpy(s_raw) * bm,
             torch.from_numpy(om[2]) * bm])[None].to(dev)
