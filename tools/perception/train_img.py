...
def s_order_loss(smap: torch.Tensor, node_pos: torch.Tensor,
    ...
    # absolute anchors: pin s(node0)->0, s(nodeLast)->1 so the ordering
    # loss also fixes WHICH end is which, not just relative monotonicity
    anchor = (sv[:, 0].abs() + (sv[:, -1] - 1.0).abs()).mean()
    return viol.mean() + anchor
