...
prev = None
flip_acc = 0.0                                       # sustained s-vs-t conflict
...
for f in episode_frames(packed, scen, seed):
    ...
    if p is None:
        ...
        # leaky integrator: sustained flip evidence commits, transient
        # ping-pong misfires cancel out
        evid = ef - eb                       # >0: reversed closer to prev
        flip_acc = flip_acc + evid if evid > 0 else flip_acc * 0.5
        if eb < ef * 0.6 or flip_acc > 0.25:
            p = p[::-1]
            flip_acc = 0.0
