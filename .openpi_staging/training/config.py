...
@dataclasses.dataclass(frozen=True)
class LeRobotCableDataConfig(DataConfigFactory):
    """Data transforms for the Panda dynamic-cable LeRobot v2.1 dataset."""

    dataset_root: str = tyro.MISSING
    episodes_index: List[int] | None = None
    default_prompt: str = "Grasp and lift the blue cable."
    action_sequence_keys: Sequence[str] = ("action",)
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/image": "observation.images.opst_cam",
        ...
        # The dataset stores absolute Cartesian pose targets. Convert xyz + Euler
        # dimensions to deltas while keeping the gripper command absolute.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotB1KDataConfig(
            ...
            base_config=DataConfig(
                prompt_from_task=True,
                episodes_index=list(range(100)),
                dataset_root="/vision/u/wsai/data/lerobot/i3l/popcorn",
            ),
            robot_config_name="i3l/RealR1Pro",
        ),
    ...
    #
    # Panda dynamic-cable pi0.5 LoRA configs. Both are single-GPU configs and
    # must be launched with CUDA_VISIBLE_DEVICES=1 on jump130.
    #
    TrainConfig(
        name="pi05_cable_lora_pilot",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=16,
