...
def _expand_sparse_episode_data_index(dataset: lerobot_dataset.LeRobotDataset) -> None:
    """Make LeRobot v0.3.x sampling work with sparse global episode ids.

    LeRobot builds ``episode_data_index`` in the order of the selected episodes,
    but ``__getitem__`` indexes it with the global episode id stored in parquet.
    Expanding the tensors preserves the global-id lookup for non-contiguous splits.
    """
    episodes = dataset.episodes
    if not episodes or max(episodes) < len(dataset.episode_data_index["from"]):
        return
    ...
    for key, values in dataset.episode_data_index.items():
        ...
        expanded[torch.as_tensor(episodes, dtype=torch.long, device=values.device)] = values
